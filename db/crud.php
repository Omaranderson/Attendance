<?php
    ...
    class crud
    {
        ...
        public function editAttendee($id, $fname, $lname, $dob, $email, $contact, $specialty)
        {
            try
            {
                $sql = "UPDATE `attendees` SET `firstname` = :fname, `lastname` = :lname, `dateofbirth` = :dob, 
        ...
        public function getAttendeeDetails($id)
        {
            try
            {
                $sql = "SELECT * FROM attendees a INNER JOIN specialties s ON a.specialty_id = s.specialty_id WHERE a.attendee_id = :id";
                $stmt = $this->db->prepare($sql);
                $stmt->bindparam(':id', $id);
                $result = $stmt->execute();
                $result = $stmt->fetch();
